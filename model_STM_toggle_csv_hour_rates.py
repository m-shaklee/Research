...
app.layout = html.Div([
    ...
    html.Div([
        ...
        html.Div([
            ...
            dcc.Slider(id='alpha', min=0.001, max=1, step=0.01, value=0.5946,
                       marks={round(i, 2): f"{i:.2f}" for i in np.linspace(0.001, 1, 6)}),
            html.Label("β: Infectivity per hour (e-8)"),
            dcc.Slider(id='beta', min=0, max=10, step=0.5, value=1.09,
                       marks={round(i, 2): f"{i:.2f}" for i in np.linspace(0, 10, 6)}),
            html.Label("δ: Infected cell death per hour (e-9)"),
            dcc.Slider(id='delta', min=0, max=10, step=0.01, value=1.98,
                       marks={round(i, 2): f"{i:.2f}" for i in np.linspace(0, 10, 6)}),
            html.Label("δ_TN: Infected cell death by TN per hour (e-9)"),
            dcc.Slider(id='delta_N', min=0, max=10, step=0.01, value=3.96,
                       marks={round(i, 2): f"{i:.2f}" for i in np.linspace(0, 10, 6)}),
            html.Label("δ_STM: Infected cell death by STM per hour (e-6)"),
            dcc.Slider(id='delta_STM',min=0, max=10, step=0.01, value=1.98,
                       marks={round(i, 2): f"{i:.2f}" for i in np.linspace(0, 10, 6)}),
...
def download_csv(n_clicks, S0, I0, TN0, STM0, alpha, beta, delta, delta_N, delta_STM, c_N, c_STM, system_mode):
    ...
    alpha=alpha*delta_t
    beta_adj = beta * 1e-8*delta_t
    delta_adj = delta * 1e-9*delta_t
    delta_N_adj = delta_N * 1e-9*delta_t
    delta_STM_adj = delta_STM * 1e-6*delta_t
...
def update_graph(S0, I0, TN0, STM0, alpha, beta, delta, delta_N, delta_STM, c_N, c_STM, system_mode):
    y0=[S0,I0,TN0,STM0,0,0,0]
    alpha=alpha*delta_t
    beta=beta*1e-8*delta_t
    delta=delta*1e-9*delta_t
    delta_N=delta_N*1e-9*delta_t
    delta_STM=delta_STM*1e-6*delta_t
