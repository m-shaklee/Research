...
app.layout = html.Div([
    ...
    html.Div([
        ...
        html.Div([
            ...
            dcc.Slider(id='beta', min=0, max=5*10**-6, step=10**-6, value=1.5*10**-6,
                       marks={round(i, 3): f"{i:.3f}" for i in np.linspace(0, 5*10**-6, 6)}),
            html.Label("δ: Infected cell death"),
            dcc.Slider(id='delta', min=0, max=0.001, step=0.0001, value=0.005,
                       marks={round(i, 2): f"{i:.2f}" for i in np.linspace(0, 0.001, 6)}),
            html.Label("δ_TN: Infected cell death by TN"),
            dcc.Slider(id='delta_N', min=0, max=0.001, step=0.0001, value=0.005,
                       marks={round(i, 2): f"{i:.2f}" for i in np.linspace(0, 0.001, 6)}),
            html.Label("δ_STM: Infected cell death by STM"),
            dcc.Slider(id='delta_STM', min=0, max=0.001, step=0.0001, value=0.005,
                       marks={round(i, 2): f"{i:.2f}" for i in np.linspace(0, 0.001, 6)}),
